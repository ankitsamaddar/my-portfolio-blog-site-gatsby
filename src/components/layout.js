...
const Layout = ({ children }) => {
  ...
  // Smooth scroll to top function using Gatsby's navigate
  const scrollToTop = () => {
  ...
  return (
    <div className="relative min-h-screen flex flex-col dark:bg-gray-800 dark:text-white bg-white text-gray-900">
      <div>
        <Header />
      </div>
      ...
      <footer className="footer items-center p-6 bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-300 border-t border-gray-200 dark:border-gray-700">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center">
          {/* Footer Text */}
          <div className="mb-4 md:mb-0 text-center md:text-left">
            <p className="text-sm">
              © {new Date().getFullYear()} <strong>Ankit Samaddar</strong>. All
              rights reserved.
            </p>
          ...
          <div className="flex flex-col items-center md:items-end">
            <nav className="flex flex-wrap gap-4 mb-2">
              <Link
                to="/"
                className="link link-hover text-gray-800 dark:text-gray-100 hover:text-blue-500 dark:hover:text-blue-400"
              >
                Home
              </Link>
              <Link
      ...
      {isVisible && (
        <div className="fixed bottom-10 right-10">
          <button
            ...
            className="btn btn-circle bg-indigo-500 text-white shadow-xl hover:bg-white transition-all w-12 h-12 flex items-center justify-center"
          >
            {/* Arrow SVG icon */}
            <svg
              className="w-full h-full text-white hover:text-indigo-500"
              fill="currentColor"
              stroke="currentColor"
              viewBox="0 0 123 123"
            >
